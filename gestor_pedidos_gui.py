...
class PedidoApp(tk.Tk):
    ...
    def ver_detalle_pedido(self, event):
        ...
        if pedido:
            ...
            top.transient(self)  # Hace que sea dependiente de la ventana principal
            top.grab_set()       # Hace la ventana modal
            top.lift()           # Trae la ventana al frente
            ...
            # Después de centrar, quitar el topmost para no ser molesto
            top.after(100, lambda: top.attributes('-topmost', False))
            ...
            # Información básica
            ttk.Label(main_frame, text=f"Empresa: {pedido.proveedor}").pack(anchor="w")
            ttk.Label(main_frame, text=f"Nº Oferta: {pedido.numero_oferta}").pack(anchor="w")
            ttk.Label(main_frame, text=f"Estado: {pedido.status}").pack(anchor="w")
            ...
            if campos_requeridos:
                ttk.Label(main_frame, text="Campos requeridos para avanzar:", font=("Arial", 10, "bold")).pack(anchor="w", pady=(10,5))
                
                if "requisition_id" in campos_requeridos:
                    ttk.Label(main_frame, text="Requisition ID:").pack(anchor="w")
                    entries["requisition_id"] = ttk.Entry(main_frame, width=50)
                    entries["requisition_id"].pack(anchor="w", padx=(20,0))
                    entries["requisition_id"].insert(0, pedido.requisition_id)
                
                if "oi" in campos_requeridos:
                    ttk.Label(main_frame, text="OI (número):").pack(anchor="w")
                    entries["oi"] = ttk.Entry(main_frame, width=50)
                    entries["oi"].pack(anchor="w", padx=(20,0))
                    entries["oi"].insert(0, str(pedido.oi) if pedido.oi > 0 else "")
                
                if "numero_pedido" in campos_requeridos:
                    ttk.Label(main_frame, text="Número de Pedido:").pack(anchor="w")
                    entries["numero_pedido"] = ttk.Entry(main_frame, width=50)
                    entries["numero_pedido"].pack(anchor="w", padx=(20,0))
                    entries["numero_pedido"].insert(0, pedido.numero_pedido)
                
                if "po_pdf" in campos_requeridos:
                    ttk.Label(main_frame, text="PDF del PO:").pack(anchor="w")
                    pdf_frame = ttk.Frame(main_frame)
                    pdf_frame.pack(anchor="w", padx=(20,0), fill="x")
                    entries["po_pdf"] = ttk.Entry(pdf_frame, width=40)
